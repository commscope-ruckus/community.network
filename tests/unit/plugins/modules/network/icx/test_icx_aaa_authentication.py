...
from ansible_collections.community.network.plugins.modules.network.icx import icx_aaa_authentication
...
class TestICXAaaAuthenticationModule(TestICXModule):
    ...
    def setUp(self):
        ...
        self.mock_load_config = patch('ansible_collections.community.network.plugins.modules.network.icx.icx_aaa_authentication.load_config')
        self.load_config = self.mock_load_config.start()
        self.mock_exec_command = patch('ansible_collections.community.network.plugins.modules.network.icx.icx_aaa_authentication.exec_command')
    ...
    def test_icx_aaa_authentication_all_options(self):
        ...
        set_module_args(dict(dot1x=dict(primary_method='radius',backup_method1='none'),
                             enable=dict(primary_method='enable',backup_method_list=['line','local','none','radius','tacacs','tacacs+']),
                             login=dict(primary_method='enable',backup_method_list=['line','local','none','radius','tacacs','tacacs+']),
                             snmp_server=dict(primary_method='enable',backup_method_list=['line','local','none' ,'radius','tacacs','tacacs+']),
                             web_server=dict(primary_method='enable',backup_method_list=['line','local','none' ,'radius','tacacs','tacacs+'])))
    ...
    def test_icx_aaa_authentication_all_options_no_backup(self):
        ''' Test for successful aaa authentication without backup_method options'''
    ...
    def test_icx_aaa_authentication_all_options_remove(self):
        ...
        set_module_args(dict(dot1x=dict(primary_method='radius',backup_method1='none',state='absent'),
                             enable=dict(primary_method='enable',backup_method_list=['line','local','none','radius','tacacs','tacacs+'],state='absent'),
                             login=dict(primary_method='enable',backup_method_list=['line','local','none','radius','tacacs','tacacs+'],state='absent'),
                             snmp_server=dict(primary_method='enable',backup_method_list=['line','local','none','radius','tacacs','tacacs+'],state='absent'),
                             web_server=dict(primary_method='enable',backup_method_list=['line','local','none','radius','tacacs','tacacs+'],state='absent')))
    ...
    def test_icx_aaa_authentication_all_options_no_backup_remove(self):
        ''' Test for removing aaa authentication without backup_method options'''
    ...
    def test_icx_aaa_authentication_dot1x_enable(self):
        ...
        set_module_args(dict(dot1x=dict(primary_method='radius',state='present'),enable=dict(primary_method='enable',backup_method_list=['line','local','none','radius','tacacs','tacacs+'],state='present')))
        expected_commands = ['aaa authentication dot1x default radius','aaa authentication enable default enable line local none radius tacacs tacacs+', 'aaa authentication enable implicit-user']
        result = self.execute_module(changed=True)
        self.assertEqual(result['commands'], expected_commands)
    ...
    def test_icx_aaa_authentication_login_snmp_server(self):
        ...
        set_module_args(dict(login=dict(primary_method='line',backup_method_list=['enable','local','none','radius','tacacs','tacacs+'],state='present'),snmp_server=dict(primary_method='enable',backup_method_list=['line','local','none','radius','tacacs','tacacs+'],state='present')))
        expected_commands = ['aaa authentication login default line enable local none radius tacacs tacacs+','aaa authentication login privilege-mode','aaa authentication snmp-server default enable line local none radius tacacs tacacs+']
        result = self.execute_module(changed=True)
        self.assertEqual(result['commands'], expected_commands)

    def test_icx_aaa_authentication_web_server(self):
        ''' Test for successful aaa authentication for web_server'''
        set_module_args(dict(web_server=dict(primary_method='tacacs',backup_method_list=['enable','line','local','none','radius','tacacs+'],state='present')))
        expected_commands = ['aaa authentication web-server default tacacs enable line local none radius tacacs+']
        result = self.execute_module(changed=True)
        self.assertEqual(result['commands'], expected_commands)
